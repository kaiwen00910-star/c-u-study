...
export function normalizeResource(row) {
  ...
  const status = row.status === 'active' ? 'published' : row.status === 'inactive' ? 'archived' : row.status
  return { ...row, status, topic_tags: tags.join('|'), tags, priority: Number(row.priority) }
}
