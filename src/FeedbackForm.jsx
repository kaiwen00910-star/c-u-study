...
export function FeedbackForm({ contextId = '', compact = false }) {
  ...
  async function handleSubmit(event) {
    ...
    if (!feedbackType || !pageUrl || !contextId || !trimmedDescription || trimmedDescription.length > 500) {
  ...
  return <details className={`feedback-panel ${compact ? 'compact' : ''}`}>
    ...
    <form name={FEEDBACK_FORM_NAME} method="POST" data-netlify="true" data-netlify-honeypot="bot-field" onSubmit={handleSubmit}>
      ...
      <label>反馈类型<select name="feedback_type" required value={feedbackType} onChange={(event) => setFeedbackType(event.target.value)}>
      ...
      <div className="feedback-actions"><small>{description.length} / 500</small><button type="submit" disabled={state === 'submitting' || !description.trim() || description.trim().length > 500}>{state === 'submitting' ? '正在提交…' : '提交反馈'}</button></div>
      <div className="feedback-status" aria-live="polite">
        {state === 'validation-error' && <p className="error">请填写 500 字以内的反馈说明，并确认页面与关联内容信息完整。</p>}
