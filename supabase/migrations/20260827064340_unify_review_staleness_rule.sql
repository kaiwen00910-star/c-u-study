-- The site uses China Standard Time for calendar-day review deadlines.
create or replace function public.review_stale_on(
  p_verified_at date,
...
set search_path = ''
as $$
  select p_verified_at is null or p_verified_at < p_as_of - 90
$$;
  ...
  where (p_query is null or concat_ws(' ', resource.title, resource.creator, resource.resource_id) ilike '%' || p_query || '%')
    ...
    and (
      ...
      or (p_filter = 'stale'
        ...
        and public.review_stale_on(resource.verified_at))
...
begin
  ...
  select jsonb_build_object(
    'issues', jsonb_build_array(
      ...
      jsonb_build_object('key','stale-offerings','label','待复核招生计划','count',(select count(*) from public.admission_offerings where status <> 'archived' and public.review_stale_on(verified_at)),'href','/admin/offerings?filter=stale'),
      jsonb_build_object('key','stale-resources','label','待复核学习资源','count',(select count(*) from public.resources where status <> 'archived' and public.review_stale_on(verified_at)),'href','/admin/resources?filter=stale'),
